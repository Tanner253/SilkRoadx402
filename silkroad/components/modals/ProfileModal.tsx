...
export function ProfileModal({ isOpen, onClose }: ProfileModalProps) {
  ...
  return (
    <div 
      className="fixed inset-0 z-50 flex items-center justify-center bg-muted p-4"
      onClick={onClose}
      style={{ marginTop: activeTab === 'sales' ? '350px' : '250px' }}
    >
      {/* Modal */}
      <div 
        className="w-full max-w-4xl max-h-[90vh] bg-card  rounded-lg shadow-xl overflow-hidden flex flex-col"
          ...
          {!loading && !error && currentTransactions.length > 0 && (
            <div className="p-6 space-y-3">
              ...
              {currentTransactions.map((tx) => (
                <div 
                  key={tx._id}
                  className="p-4 border border-border  rounded hover:bg-muted "
                >
